...
class P8MArray(MatrixGArray):
    ...
    def __init__(self, data, p='int'):
        ...
        print(data.shape[-1])
        assert (p == 'int' and data.shape[-1] == 4, 8) or (p == 'hmat' and data.shape[-2:] == (3, 3)) # r data
...
def identity(shape=(), p='int'):
    e = P8MArray(np.zeros(shape + (4,), dtype=np.int), 'int')
...
def rand(minu, maxu, minv, maxv, size=()):
    ...
    return P8MArray(data=data, p='int')
...
def rotation(r, center=(0, 0)):
    ...
    rdata = np.zeros(r.shape + (8,), dtype=np.int)
    rdata[..., 1] = r
    r0 = P8MArray(rdata)

    tdata = np.zeros(center.shape[:-1] + (8,), dtype=np.int)
    tdata[..., 2:] = center
    t = P8MArray(tdata)
...
def r_range(start=0, stop=8, step=1):
    ...
    assert start < 8
    assert start < stop
    m = np.zeros((stop - start, 8), dtype=np.int)
